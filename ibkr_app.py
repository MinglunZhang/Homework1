...
while True:
    ...
    if 'currency_pair.txt' in listdir():
        ...
        # Code goes here...
        df = util.df(bars)
        df.to_csv('currency_pair_history.csv', index=False)
    ...
    if 'trade_order.p' in listdir():
        ...
        contract = Forex(trd_ordr.get("trade_currency"))
        order = MarketOrder(trd_ordr.get("action"), trd_ordr.get("trade_amt"))
